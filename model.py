...
from bionet.preparation import (#as_perturbation_fn, as_greyscale_perturbation_fn, 
                                ...
                                invert_luminance)
from bionet.bases import BioResNet50, allcnn, allcnn_imagenet, retinal_bottleneck, recurrent_connections_LSTM_back, recurrent_connections_LSTM
...
parser.add_argument('--recurrent', action='store_true',
                    help='Flag to add recurrent connections')
parser.add_argument('--bottleneck', type=int, default=None,
                    help='Number of channels for the retinal bottleneck. If not set, the bottleneck is not used.')
parser.add_argument('--continue_train', action='store_true',
                    help='Flag to continue training with the last weights stored')
...
# Retinal Information Bottleneck incorporation
if bottleneck_width != None:
    model = retinal_bottleneck(model, bottleneck_width)

# Adding recurrent connections
if recurrent:
  model = recurrent_connections_LSTM(model)
...
#             class_mode='categorical',
    ...
    if os.path.exists(model_data_file) and not clean:
        ...
        if continue_training:
            ...
            latest_checkpoint = tf.train.latest_checkpoint(models_dir)
